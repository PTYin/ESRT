...
for (( i = 0; i < 6; i++ )); do
  for (( j = 0; j < 4; j++ )); do
    python main.py --setting_file "config/${dataset[i]}/${models[j]}.yaml" >> "/home/share/yinxiangkun/log/${embedding_size}/${dataset[i]}/${models}/train_log.txt" &&
    python main.py --setting_file "config/${dataset[i]}/${models[j]}.yaml" --decode True >> "/home/share/yinxiangkun/log/${embedding_size}/${dataset[i]}/${models}/test_log.txt" &&
