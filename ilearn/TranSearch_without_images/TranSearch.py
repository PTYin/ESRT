...
class TranSearch(object):
	def __init__(self, text_size, embed_size, item_size,
	...
	def convert_to_space(self, text, vis=None):
	...
	def create_model(self):
		...
		if self.mode == 'vis':
			...
			self.pos_text_conv = self.convert_to_space(self.pos_text)
			self.neg_text_conv = self.convert_to_space(self.neg_text)
	...
	def step(self, sess, sample, all_items_embed, step):
		...
		if self.is_training:
			...
			input_feed[self.neg_text.name] = sample[3]
